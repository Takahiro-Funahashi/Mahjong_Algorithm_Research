...
class Test_Haipai(Pai.class_MJ_SheyPai):
    ...
    def create_dialog(self):
        ...
        # 牌選択エリアの大きさ
        c_width = PaiIMG.PAI_SIZE[W]*(PaiIMG.NUM_PAI+1) + \
            self.blank*(PaiIMG.NUM_PAI)+self.offset*2
        c_height = PaiIMG.PAI_SIZE[H]*(PaiIMG.NUM_PATTERN) + \
            self.blank*(PaiIMG.NUM_PATTERN-1)+self.offset*2
        # 手牌エリアの大きさ
        ...
        self.view_dialog.geometry(
            f'{max(b_width,c_width,h_width,n_width)}x{b_height*2+c_height+h_height+n_height+5*4}+300+300')
    ...
    def _haipai_farame_(self):
        ...
        for i in range(14):
            x0 = self.offset+PaiIMG.PAI_SIZE[W] * \
                i+(self.blank*2)*(i+int(i/(14-1))*2)
            y0 = self.offset
    ...
    def _draw_haipai_(self, pai_index, pos_index):
        ...
        x = self.offset+PaiIMG.PAI_SIZE[W]*pos_index + \
            (self.blank*2)*(pos_index+int(pos_index/(14-1))*2)
        y = self.offset
...
class PopCheySelect():
    def __init__(self, parent, image_set):
        ...
        c_height = PaiIMG.PAI_SIZE[H] * \
            (number)+self.blank*(number)+self.offset*2
        ...
        btn_Cancel = tk.Button(
            InFrame2, width=8, text='Cancel', command=self.Cancel)
        ...
        for j, pt in enumerate(image_set):
            ...
            for i, img in enumerate(pt):
                ...
                if i == 0:
                    ...
                    y = self.offset + \
                        PaiIMG.PAI_SIZE[H]*j + self.blank*2*j + \
                        (PaiIMG.PAI_SIZE[H]-PaiIMG.PAI_SIZE[W])
                else:
